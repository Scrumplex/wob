...
{
	char *actual_config_path;
	if (config_path == NULL) {
		actual_config_path = default_config_path();
	}
	else {
		actual_config_path = strdup(config_path);
	}

	int res = ini_parse(actual_config_path, handler, config);

	if (res == -1 || res == -2) {
		wob_log_error("Failed to open config file %s", actual_config_path);

		free(actual_config_path);
		return false;
	}

	if (res != 0) {
		wob_log_error("Failed to parse config file %s, error at line %d", actual_config_path, res);
